...
Utils.getEmailAndToken = async (user_file_path, endpoint = "autocorector") => {
    ...
    if (await Utils.checkFileExists(path.resolve(process.cwd(), user_file_path))) {
        try{
            ...
            return Promise.reject(new Error("El fichero con la información del usuario contiene información inválida. La dirección de correo electrónico debe ser del dominio upm.es"));
        }
    }
    ...
    const email_prompt = isNewMiriada ? "Escriba su email con el que accede a MiriadaX:  ":"Escriba su email de alumno UPM:  "
    const token_prompt = isNewMiriada ? "Escriba su token de MiriadaX:  ":"Escriba su token de Moodle UPM: "
